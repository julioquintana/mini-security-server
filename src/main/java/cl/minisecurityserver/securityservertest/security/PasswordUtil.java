package cl.minisecurityserver.securityservertest.security;
...
public class PasswordUtil {
  ...
  /**
   * This method can be used to generate a string representing an account pass suitable for storing
   * in a database. It will be an OpenBSD-style crypt(3) formatted hash string of length=60 The
   * bcrypt workload is specified in the above static variable, a value from 10 to 31. A workload of
   * 12 is a very reasonable safe default as of 2013. This automatically handles secure 128-bit salt
   * generation and storage within the hash.
   *
   * @param passwordPlainText The account's plaintext pass as provided during account creation, or
   *     when changing an account's pass.
   ...
   * request) with that of a stored hash from a database. The pass hash from the database must be
